...
class SVMScale {
	private static final String TAG = "SVMScale";
	...
	private void output(int index, double value) {
		...
		Log.d(TAG, "previous value : " + value);
	...
	public void run(String inputFile, String restoreFile, String outputFile,
		...
		for (i = 0; i < optionLen; i++) {
			...
			case 'l':
				lower = Double.parseDouble(option[i]);
				break;
			case 'u':
				upper = Double.parseDouble(option[i]);
				break;
			case 'y':
				y_lower = Double.parseDouble(option[i]);
				++i;
				y_upper = Double.parseDouble(option[i]);
				y_scaling = true;
				break;
			case 's':
				save_filename = option[i];
				break;
			case 'r':
				restore_filename = option[i];
				break;
			default:
		...
		if (save_filename != null) {
			...
			for (i = 1; i <= max_index; i++) {
				Log.d(TAG, "feature_min[i] : " + feature_min[i]
						+ ", feature_max[i] : " + feature_max[i]);
		...
		/* pass 3: scale */
		//BufferedWriter out = new BufferedWriter(new FileWriter(data_filename));
		while (readline(fp) != null) {
			int next_index = 1;
			double target;
			double value;
